//! Type for representing a disparity frame taken from a depth camera.
...
impl<'a> DisparityFrame<'a> {
    pub fn iter(&'a self) -> ImageIter<'a, DisparityFrame<'a>> {
        ImageIter {
            frame: self,
            column: 0,
            row: 0,
...
impl<'a> Drop for DisparityFrame<'a> {
    fn drop(&mut self) {
        unsafe {
...
impl<'a> std::convert::TryFrom<NonNull<sys::rs2_frame>> for DisparityFrame<'a> {
...
impl<'a> VideoFrameUnsafeEx<'a> for DisparityFrame<'a> {
...
impl<'a> VideoFrameEx<'a> for DisparityFrame<'a> {
    fn width(&self) -> usize {
        self.width
    }
...
impl<'a> IntoIterator for &'a DisparityFrame<'a> {
    type Item = <ImageIter<'a, DisparityFrame<'a>> as Iterator>::Item;
    type IntoIter = ImageIter<'a, DisparityFrame<'a>>;
